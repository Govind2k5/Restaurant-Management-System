...
public class WaiterDashboard extends JFrame {
    ...
    public WaiterDashboard(User user) {
        this.user = user;
        this.ctrl = new WaiterController();
        initUI();
    }

    private void initUI() {
        setTitle("Restaurant MS — Waiter Dashboard");
        setSize(1140, 680);
        ...
        // Live stats
        int availTables = 0, menuItems = 0, totalTables = 0;
        try { availTables = ctrl.getAvailableTables().size();     } catch (Exception ignored) {}
        try { menuItems   = ctrl.getAvailableMenuItems().size();  } catch (Exception ignored) {}
        try { totalTables = ctrl.getAllTables().size();            } catch (Exception ignored) {}

        root.add(buildSidebar(), BorderLayout.WEST);
        root.add(buildContent(availTables, menuItems, totalTables), BorderLayout.CENTER);
    ...
    private JPanel buildContent(int availTables, int menuItems, int totalTables) {
        ...
        statsRow.add(buildStatCard("🪑", String.valueOf(availTables), "Available Tables",  UITheme.ACCENT2));
        statsRow.add(buildStatCard("🍴", String.valueOf(menuItems),   "Items on Menu",     UITheme.ACCENT));
        statsRow.add(buildStatCard("📋", String.valueOf(totalTables), "Total Tables",      UITheme.ACCENT3));
        center.add(statsRow, BorderLayout.NORTH);

        // Feature cards
        JPanel cards = new JPanel(new GridLayout(1, 3, 14, 0));
        cards.setBackground(UITheme.BG_DEEP);
        cards.add(buildFeatureCard("🛒", "Place Order",
            "Select a table, browse the menu,\nadd items to cart and submit.\nObserver Pattern notifies the Chef's KDS.",
            UITheme.ACCENT2, () -> new OrderView(ctrl, user)));
        cards.add(buildFeatureCard("🪑", "Table Reservations",
            "View all dining tables and status.\nMark tables as reserved or available.\nCapacity information included.",
            UITheme.ACCENT3, () -> new TableReservationView(ctrl)));
        cards.add(buildFeatureCard("✕", "Cancel Order",
            "Cancel an active order by its ID.\n«extends» the Place Order use case.\nUpdates Chef's KDS automatically.",
            UITheme.DANGER, () -> showCancelDialog()));
        center.add(cards, BorderLayout.CENTER);

        content.add(center, BorderLayout.CENTER);

        // Bottom tip + status bar combined
        JPanel south = new JPanel(new BorderLayout());
        south.setBackground(UITheme.BG_DEEP);
        JPanel tip = buildTip("💡  After placing an order, the Chef's Kitchen Display updates instantly via the Observer Pattern.");
        south.add(tip, BorderLayout.NORTH);
        south.add(buildStatusBar("Waiter Module  |  Singleton & Observer Patterns active"), BorderLayout.SOUTH);
    ...
    private JPanel buildHeader() {
        ...
        title.setForeground(UITheme.ACCENT2);
    ...
    private JPanel buildSidebar() {
        ...
        top.add(buildProfileCard(user, UITheme.ACCENT2, sidebarBg));
        top.add(buildDivider());

        JButton orderBtn  = sidebarButton("🛒  Place Order",        UITheme.ACCENT2);
        JButton tableBtn  = sidebarButton("🪑  Table Reservations",  UITheme.ACCENT3);
        JButton cancelBtn = sidebarButton("✕   Cancel Order",        UITheme.DANGER);
        orderBtn.addActionListener(e  -> new OrderView(ctrl, user));
        tableBtn.addActionListener(e  -> new TableReservationView(ctrl));
        ...
        nav.add(orderBtn); nav.add(tableBtn); nav.add(cancelBtn);
        top.add(nav);
    ...
    private JPanel buildTip(String text) {
        JPanel tip = new JPanel(new BorderLayout());
        tip.setBackground(new Color(16, 22, 38));
    ...
    private void showCancelDialog() {
        String inp = JOptionPane.showInputDialog(this, "Enter Order ID to cancel:", "Cancel Order", JOptionPane.WARNING_MESSAGE);
        if (inp != null && !inp.isBlank()) {
            try {
                boolean ok = ctrl.cancelOrder(Integer.parseInt(inp.trim()));
                if (ok) JOptionPane.showMessageDialog(this, "Order #" + inp + " has been cancelled.", "Cancelled", JOptionPane.INFORMATION_MESSAGE);
                else    JOptionPane.showMessageDialog(this, "Could not cancel. Check the order ID.", "Error", JOptionPane.ERROR_MESSAGE);
