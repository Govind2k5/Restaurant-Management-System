...
import com.restaurant.view.chef.InventoryView;
...
public class AdminDashboard extends JFrame {
    ...
    private ChefController chefCtrl;

    public AdminDashboard(User user) {
        this.currentUser = user;
        this.ctrl = new AdminController();
        this.chefCtrl = new ChefController();
        initUI();
    }

    private void initUI() {
        setTitle("Restaurant MS — Admin Dashboard");
        setSize(1140, 680);
        ...
        // Live stats (safe — falls back to 0 on DB error)
        int empCount = 0, menuCount = 0, inventoryCount = 0;
        try { empCount       = ctrl.getAllEmployees().size();     } catch (Exception ignored) {}
        try { menuCount      = ctrl.getAllMenuItems().size();     } catch (Exception ignored) {}
        try { inventoryCount = chefCtrl.getAllInventory().size(); } catch (Exception ignored) {}

        root.add(buildSidebar(), BorderLayout.WEST);
        root.add(buildContent(empCount, menuCount, inventoryCount), BorderLayout.CENTER);
    ...
    private JPanel buildContent(int empCount, int menuCount, int inventoryCount) {
        ...
        statsRow.add(buildStatCard("👥", String.valueOf(empCount),       "Total Staff",     UITheme.ACCENT3));
        statsRow.add(buildStatCard("🍴", String.valueOf(menuCount),      "Menu Items",      UITheme.ACCENT));
        statsRow.add(buildStatCard("📦", String.valueOf(inventoryCount), "Inventory Items", UITheme.ACCENT2));
        center.add(statsRow, BorderLayout.NORTH);

        // Feature cards
        JPanel cards = new JPanel(new GridLayout(1, 3, 14, 0));
        cards.setBackground(UITheme.BG_DEEP);
        cards.add(buildFeatureCard("🍴", "Menu Management",
            "Add, edit, and delete food & beverage items.\n" +
            "Toggle item availability in real-time.\n" +
            "Factory Pattern creates typed MenuItem objects.",
            UITheme.ACCENT, () -> new MenuManagementView(ctrl)));
        cards.add(buildFeatureCard("👥", "Employee Management",
            "Onboard staff with login credentials.\n" +
            "Assign roles: Waiter, Chef, or Manager.\n" +
            "Atomic DB transaction ensures data integrity.",
            UITheme.ACCENT3, () -> new EmployeeManagementView(ctrl)));
        cards.add(buildFeatureCard("📦", "Inventory Management",
            "View and update ingredient stock levels.\n" +
            "Add new inventory items with quantities.\n" +
            "Color-coded alerts for low / out-of-stock.",
            UITheme.ACCENT2, () -> new InventoryView(chefCtrl)));
        center.add(cards, BorderLayout.CENTER);

        content.add(center, BorderLayout.CENTER);
        content.add(buildStatusBar("Admin Module  |  Factory & Singleton Patterns active"), BorderLayout.SOUTH);
        return content;
    }
    ...
    private JPanel buildHeader() {
        ...
        JLabel title = new JLabel(greeting + ", " + currentUser.getName());
    ...
    private JPanel buildSidebar() {
        ...
        top.add(buildProfileCard(currentUser, UITheme.ACCENT, sidebarBg));
        top.add(buildDivider());

        JButton menuBtn = sidebarButton("🍴  Menu Management", UITheme.ACCENT);
        JButton empBtn  = sidebarButton("👥  Employees",        UITheme.ACCENT3);
        JButton invBtn  = sidebarButton("📦  Inventory",         UITheme.ACCENT2);
        menuBtn.addActionListener(e -> new MenuManagementView(ctrl));
        empBtn.addActionListener(e  -> new EmployeeManagementView(ctrl));
        invBtn.addActionListener(e  -> new InventoryView(chefCtrl));
    ...
    private JPanel buildProfileCard(User user, Color accent, Color bg) {
