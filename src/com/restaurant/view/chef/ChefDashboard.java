...
public class ChefDashboard extends JFrame {
    ...
    public ChefDashboard(User user) {
        this.user = user;
        this.ctrl = new ChefController();
        initUI();
    }

    private void initUI() {
        setTitle("Restaurant MS — Chef Dashboard");
        setSize(1140, 680);
        ...
        // Live stats
        int pendingOrders = 0, inProgressOrders = 0, inventoryItems = 0;
        try { pendingOrders    = ctrl.getPendingOrders().size();     } catch (Exception ignored) {}
        try { inProgressOrders = ctrl.getInProgressOrders().size();  } catch (Exception ignored) {}
        try { inventoryItems   = ctrl.getAllInventory().size();       } catch (Exception ignored) {}

        root.add(buildSidebar(), BorderLayout.WEST);
        root.add(buildContent(pendingOrders, inProgressOrders, inventoryItems), BorderLayout.CENTER);
    ...
    private JPanel buildContent(int pendingOrders, int inProgressOrders, int inventoryItems) {
        ...
        statsRow.add(buildStatCard("⏳", String.valueOf(pendingOrders),    "Pending Orders",     UITheme.ACCENT4));
        statsRow.add(buildStatCard("🔥", String.valueOf(inProgressOrders), "In Progress",        UITheme.ACCENT));
        statsRow.add(buildStatCard("📦", String.valueOf(inventoryItems),   "Inventory Items",    UITheme.ACCENT2));
        center.add(statsRow, BorderLayout.NORTH);

        // Feature cards
        JPanel cards = new JPanel(new GridLayout(1, 2, 14, 0));
        cards.setBackground(UITheme.BG_DEEP);
        cards.add(buildFeatureCard("🖥", "Kitchen Display (KDS)",
            "Real-time order queue from all waiters.\n" +
            "Update status: Pending → In Progress → Ready.\n" +
            "Observer Pattern auto-refreshes this view.",
            UITheme.ACCENT4, () -> openKDS()));
        cards.add(buildFeatureCard("📦", "Inventory Tracking",
            "View and update ingredient stock levels.\n" +
            "Toggle menu item availability (Out of Stock).\n" +
            "Keeps the menu accurate and up to date.",
            UITheme.ACCENT2, () -> new InventoryView(ctrl)));
        ...
        JPanel tip = buildTip("💡  Opening the Kitchen Display registers this Chef as an Observer — new orders appear automatically without refreshing.");
        south.add(tip, BorderLayout.NORTH);
        south.add(buildStatusBar("Chef Module  |  Observer Pattern active"), BorderLayout.SOUTH);
    ...
    private JPanel buildHeader() {
        ...
        JLabel title = new JLabel(greeting + ", Chef " + user.getName());
        title.setFont(new Font("Segoe UI", Font.BOLD, 20));
        title.setForeground(UITheme.ACCENT4);
    ...
    private JPanel buildSidebar() {
        ...
        top.add(buildProfileCard(user, UITheme.ACCENT4, sidebarBg));
        top.add(buildDivider());

        JButton kdsBtn = sidebarButton("🖥  Kitchen Display",  UITheme.ACCENT4);
        JButton invBtn = sidebarButton("📦  Inventory",        UITheme.ACCENT2);
        kdsBtn.addActionListener(e -> openKDS());
        invBtn.addActionListener(e -> new InventoryView(ctrl));

        JPanel nav = new JPanel(new GridLayout(0, 1, 0, 4));
        nav.setBackground(sidebarBg);
        nav.setBorder(new EmptyBorder(10, 10, 10, 10));
        nav.add(kdsBtn); nav.add(invBtn);
        top.add(nav);
    ...
    private void openKDS() {
        KitchenDisplayView v = new KitchenDisplayView(ctrl);
        ctrl.registerKitchenDisplay(v);
    }
